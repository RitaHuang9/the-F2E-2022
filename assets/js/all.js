...
$(function () {
  ...
  mm.add({
    isDesktop: "(min-width: ".concat(breakPoint, "px) and (prefers-reduced-motion: no-preference)"),
    isMobile: "(max-width: ".concat(breakPoint - 1, "px) and (prefers-reduced-motion: no-preference)")
  }, function (context) {
    var _context$conditions = context.conditions,
        isDesktop = _context$conditions.isDesktop,
        isMobile = _context$conditions.isMobile; // -------------------------------------------------------------------> TODO: 第一屏

    var cloud = gsap.timeline({
      scrollTrigger: {
        trigger: ".section02",
    ...
    blockStart.to('.traffic-light-txt', {
      //1.ready文字消失
      opacity: 0
      ...
      //4.綠燈[亮]，其他燈[暗]，Ready改成GO!
      opacity: 1
    }).to('.traffic-light-yellow', {
      opacity: 0
    }, '<').to('.traffic-light-txt', {
      opacity: 1
    }, '<').to(['.traffic-light-txt', '.traffic-light-green', '.traffic-light-frame', '.job-title', '.cloud-left', '.cloud-right'], {
      //5.整個紅綠燈、參賽人數消失
      opacity: 0
    }).to(['.railing-start', '.section01 .logo', '.title-main'], {
      //6.欄杆、大logo消失、左側小logo顯示
      opacity: 0
    }).to('.logo-s', {
    ...
    var blockSeventh = gsap.timeline({
      scrollTrigger: {
        trigger: ".section07",
        // markers: true,
        pin: true,
    ...
    blockSeventh.to('.section07 .title-sub', {
      ...
      yPercent: '-10',
      opacity: 1
    }, '<0.1').to('.sponsor-kdanmobile', {
      yPercent: '-10',
      opacity: 1
    }, '<0.2').to(['.characters-tree-left', '.characters-tree-right', '.section07 .title-sub'], {
      opacity: 0
    }).to('.characters-tree-left', {
      width: '-=2%',
      left: '20%',
